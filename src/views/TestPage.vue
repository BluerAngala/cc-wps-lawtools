    ...
    <div class="p-4 space-y-4">
      ...
      <n-card title="WPS 加载项调试工具" size="small">
        ...
        <n-space vertical size="small">
          <n-text depth="3"> 按 <n-tag size="tiny">F12</n-tag> 打开调试器 </n-text>
          <n-text depth="3">
      ...
      <n-card title="功能测试面板" size="small">
        <n-collapse accordion>
          ...
          <n-collapse-item title="🖼️ 窗格控制" name="paneControl">
            <n-space vertical size="medium">
              <n-space>
                <n-button @click="executeTest('dockLeft')" size="small"> ⬅️ 停靠左边 </n-button>
                <n-button @click="executeTest('dockRight')" size="small"> ➡️ 停靠右边 </n-button>
                <n-button @click="executeTest('hideTaskPane')" size="small"> 👁️ 隐藏窗格 </n-button>
              </n-space>
              <!-- 结果预览 -->
      ...
      <n-card v-if="extractedText" title="📝 文档文本内容" size="small">
        <pre class="text-sm whitespace-pre-wrap" style="max-height: 240px; overflow-y: auto">{{
          extractedText
        }}</pre>
      </n-card>
      ...
      <n-card v-if="sensitiveInfoList.length > 0" title="🔒 检测到的敏感信息" size="small">
        <n-space vertical>
            ...
            <n-list-item v-for="(info, index) in sensitiveInfoList" :key="index">
              <n-thing :title="info.type">
                <template #description>
                  <n-space vertical size="small">
                    <n-text
                      >原始: <n-tag type="error" size="small">{{ info.original }}</n-tag></n-text
                    >
                    <n-text
                      >脱敏:
                      <n-tag type="success" size="small">{{ info.desensitized }}</n-tag></n-text
                    >
                  </n-space>
                </template>
              </n-thing>
          ...
          <n-space>
            ...
            <n-button @click="sensitiveInfoList = []" size="small"> 取消 </n-button>
          </n-space>
        </n-space>
      </n-card>
    </div>
  ...
  try {
    ...
    switch (functionName) {
      case 'desensitizeText': {
        ...
        testResults.value[category] =
          `已处理脱敏，检测到 ${sensitiveInfoList.value.length} 个敏感信息`
        break
      }
...
const handleAIProcess = async (processFunc) => {
  try {
    ...
    unifiedLogger.error('AI处理失败，请查看控制台错误信息', {
      method: 'handleAIProcess',
      error: error.message
    })
  }
}
  ...
  await handleAIProcess(async () => {
    ...
    const handleTaskError = (task, error) => {
      if (task.id === taskId) {
        console.error('AI处理失败:', error)
        unifiedLogger.error('AI处理失败，请查看控制台错误信息', {
          method: 'onProcessUserTextWithAI',
          taskId,
          error: error.message
        })
...
const processDesensitizeText = (text) => {
  ...
  const { desensitizedText: resultText, sensitiveInfoList: resultList } =
