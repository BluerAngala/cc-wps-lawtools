...
- **searchKeyword字段（关键）**：必须是从当前条款中提取的**简短、精确的文本片段**（5-20个字符），用于在文档中定位。要求：
  1. 必须是文档中实际存在的连续文本
  2. 尽量选择独特的、不容易重复的短语
  3. 避免包含标点符号和特殊字符
  4. 优先选择问题所在的具体文字
  5. 如果原文是"邀请行业翘楚、电影专家担任培训老师进行授课"，searchKeyword可以是"电影专家担任培训老师"或"行业翘楚、电影专家"
- **keyword字段**：保留完整的问题文本片段（用于显示，可以较长）
...
请以JSON格式返回审查结果：
{
  "issues": [
    {
      "searchKeyword": "用于定位的简短精确文本（5-20字符，必须存在于文档中）",
      "keyword": "完整的问题文本片段（用于显示）",
      "comment": "问题描述和建议",
