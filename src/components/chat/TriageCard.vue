  ...
  <div class="triage-card" :class="`level-${level.toLowerCase()}`">
    ...
    <div v-if="issues.length" class="tc-issues">
      <div v-for="(issue, i) in issues" :key="i" class="tc-issue" :class="`sev-${issue.severity}`">
        ...
        <span class="ti-sev" :class="`sev-${issue.severity}`">{{
...
.triage-card.level-green {
  border-color: #16a34a;
  background: #f0fdf4;
}
.triage-card.level-yellow {
  border-color: #d97706;
  background: #fffbeb;
}
.triage-card.level-red {
  border-color: #e63946;
  background: #fef2f2;
}
.tc-header {
...
.tc-level-tag.tag-green {
  background: #dcfce7;
  color: #166534;
}
.tc-level-tag.tag-yellow {
  background: #fef9c3;
  color: #854d0e;
}
.tc-level-tag.tag-red {
  background: #fecaca;
  color: #991b1b;
}
.tc-summary {
...
.tc-issue.sev-high .ti-dot {
  background: #e63946;
}
.tc-issue.sev-medium .ti-dot {
  background: #f5c518;
}
.tc-issue.sev-low .ti-dot {
  background: #16a34a;
}
.ti-body {
  flex: 1;
