    ...
    <div v-if="items.length" class="cv-items">
      <div v-for="(item, i) in items" :key="i" class="cv-item" :class="`chg-${item.change}`">
        <div class="ci-header">
          ...
          <span class="ci-change-tag" :class="`chg-${item.change}`">{{
...
function changeLabel(c) {
  return { modified: '已修改', added: '新增', removed: '已删除' }[c] || c
}
...
.ci-change-tag.chg-modified {
  background: #fef9c3;
  color: #854d0e;
}
.ci-change-tag.chg-added {
  background: #dbeafe;
  color: #1e40af;
}
.ci-change-tag.chg-removed {
  background: #fecaca;
  color: #991b1b;
}
.ci-diff {
