...
watch(() => props.showSlashMenu, (v) => { if (v) activeCmdIdx.value = 0 })
...
watch(() => props.modelValue, () => nextTick(autoResizeInput))
...
.slash-menu {
  position: absolute; bottom: 100%; left: 6px; right: 6px;
  background: var(--c-surface); border: 1px solid var(--c-border);
  border-radius: var(--radius-sm, 8px); box-shadow: 0 -4px 16px rgba(0,0,0,0.1);
  max-height: 260px; overflow-y: auto; z-index: 10; padding: 6px;
}
.slash-title { font-size: 11px; font-weight: 700; color: var(--c-text2); padding: 4px 10px 6px; text-transform: uppercase; letter-spacing: 0.5px; }
.slash-item {
  display: flex; align-items: center; gap: 10px;
  width: 100%; padding: 8px 10px; border: none; background: transparent;
  border-radius: 6px; cursor: pointer; text-align: left; transition: background 0.12s;
}
...
.chat-input {
  flex: 1; resize: none;
  border: 1.5px solid var(--c-border); border-radius: var(--radius-sm, 8px);
...
.chat-input::placeholder { color: var(--c-text2); }
.send-btn {
  width: 34px; height: 34px; border: none; border-radius: 8px;
  background: var(--c-border); color: #fff; cursor: pointer;
...
.stop-btn {
  width: 34px; height: 34px; border: 1.5px solid var(--c-accent);
  border-radius: 8px; background: transparent; color: var(--c-accent);
  cursor: pointer; display: flex; align-items: center; justify-content: center;
  flex-shrink: 0; transition: all 0.2s;
}
.stop-btn:hover { background: #f5f5f5; }
.send-spin {
...
.input-hint {
  ...
  padding: 2px 2px 0; font-size: 10px; color: var(--c-text2);
}
