      ...
      >
        <div class="ri-header">
          ...
          <span class="ri-levels">
            <span class="ri-sev" :class="`sev-${item.severity}`">{{
              sevLabel(item.severity)
            }}</span>
            <span class="ri-lik" :class="`lik-${item.likelihood}`">{{
              likLabel(item.likelihood)
            }}</span>
          </span>
        </div>
...
const displayItems = computed(() => {
  ...
  return props.items.filter(
    (i) => i.severity === filter.value.sev && i.likelihood === filter.value.lik
  )
})
...
function sevLabel(s) {
  return { high: '高严重', medium: '中严重', low: '低严重' }[s] || s
}
function likLabel(l) {
  return { high: '高可能', medium: '中可能', low: '低可能' }[l] || l
}
</script>
